...
namespace breakfastquay {
	...
	public:
		...
		vector<double> getTempoCandidates() const
		{
			return m_candidates;
		}
		...
		double finish()
		{
			...
			multimap<double, int> candidateMap;
			for (int i = 1; i + 1 < cflen; ++i) {
				if (cf[i] > cf[i - 1] && cf[i] > cf[i + 1]) {
					candidateMap.insert(pair<double, int>(cf[i], i));
				}
			}
			...
			multimap<double, int>::const_iterator ci(candidateMap.end());
	...
	private:
		...
		vector<double> m_lfdf;
		vector<double> m_hfdf;
		vector<double> m_rms;

		vector<double> m_candidates;
