...
void BeatDetector::loadSongToDelay(int milliseconds)
{
	...
	songChannel2->setVolume(volume);
	timeToDelay = milliseconds;

	songChannel2->setPaused(true);
	areWePlaying = true;

	cout << "Song Length: " << minutes << ":" << seconds << endl;
	cout << "Sample Rate: " << sampleRate << endl;
	cout << "Freq Range: " << hzRange << endl;
}
...
float* BeatDetector::getCurrentSpectrum()
{
	...
	for (int i = 0; i < sampleSize / 2; ++i)
	{
		...
		if (tempSpec[i] > dominantValue) dominantValue = tempSpec[i];
	}
...
float * BeatDetector::getCurrentSpectrumTwo()
{
	float *specLeft, *specRight, *tempSpec;
	specLeft = new float[sampleSize];
	specRight = new float[sampleSize];
	...
	//Get Spectrum of Song Channel for left and right
	songChannel2->getSpectrum(specLeft, sampleSize, 0, FMOD_DSP_FFT_WINDOW_HAMMING);
	songChannel2->getSpectrum(specRight, sampleSize, 1, FMOD_DSP_FFT_WINDOW_HAMMING);
...
float BeatDetector::calculateFluxAndSmoothing(float* currentSpectrum)
{
	...
	{
		sort(spectrumFluxes.begin(), spectrumFluxes.end());
		sort(smootherValues.begin(), smootherValues.end());
	...
	//Smoothing for different averages.  Lower = more, Higher = less sensitive.
	if (smoothMedian < 0.1) thresholdSmoother = 0.11f;
	if (smoothMedian >= 0.1 && smoothMedian < 0.5) thresholdSmoother = 0.17f;
	if (smoothMedian >= 0.5 && smoothMedian < 1) thresholdSmoother = 0.37f;
	if (smoothMedian >= 1 && smoothMedian < 2) thresholdSmoother = 0.42f;  //Originally 0.8f
	if (smoothMedian >= 2 && smoothMedian < 3) thresholdSmoother = 0.7f;  //Org. 1
	if (smoothMedian >= 3 && smoothMedian < 4) thresholdSmoother = 0.8f;  //Org. 1
	if (smoothMedian >= 4 && smoothMedian < 5) thresholdSmoother = 1.0f;   //Org. 2.2
	if (smoothMedian >= 5 && smoothMedian < 6) thresholdSmoother = 1.5f;   //Org. 2.2
	if (smoothMedian > 6) thresholdSmoother = 2.0f;		//Org. 2.4
...
void BeatDetector::update()
{
	...
	if (started)
	{
		...
		mbpm.process(specStereo, 4);
		double tempDouble = mbpm.estimateTempo();

		double tempo = 60000 / tempDouble;

		if (tempo < 150) tempo = 150;
		else if (tempo > 245) tempo = 245;

		//Beat detected
		if (specFlux > beatThreshold && (clock() - timeBetween) >= 250) //default 245, might get rid of tempo
		{
			...
			cout << "tempDouble: " << tempDouble << ", tempo: " << tempo << endl;

			timeBetween = clock();

			TimeStamp* t;
			if (!delayedSong)
				t = new TimeStamp(currentMinutes, currentSeconds, currentMillis, specFlux, dominantValue, timeToDelay);
			else
			{
				int tempMin, tempSec, tempMillis;
				tempMillis = currentMillis - timeToDelay;
				tempSec = currentSeconds;
				tempMin = currentMinutes;
				if (tempMillis < 0)
				{
					tempSec--;
					tempMillis += 1000;
				...
				t = new TimeStamp(tempMin, tempSec, tempMillis, specFlux, dominantValue, timeToDelay);
			}
			cout << "BEAT AT: "
				<< t->getMinutes() << ":"
				<< t->getSeconds() << ":"
				<< t->getMilliseconds() << " - BEATFREQ: "
				<< t->getFrequency() << " - THRESHOLD: "
				<< beatThreshold << " - DOMFREQ: "
				<< dominantValue << " - SMOOTHMED: "
				<< smoothMedian << " - SMOOTHER: "
				<< thresholdSmoother << endl;
			lastBeatRegistered = t;
		}
		else if ((clock() - timeBetween) >= 4000 && !pausedOrNot)
		{
			cout << "It stopped here!" << endl;
			if (thresholdSmoother > 0.6f)  //org. 0.4f
				thresholdSmoother -= 0.6f;
		...
		if (!killSong)
		{
			...
			else if (delayedSong && clock() - fullSeconds > timeToDelay)
				songChannel2->isPlaying(&areWePlaying);
			else if (delayedSong && clock() - fullSeconds <= timeToDelay)
				areWePlaying = true;
		}
		else areWePlaying = false;

		if (!areWePlaying)
		{
			cout << "FINI! \n";
			cout << "FINAL BPM: " << tempDouble << endl;
			if (currentMinutes >= (int)minutes && currentSeconds >= (int)seconds) completeSong = true;
		}
		delete[] specStereo;
		dominantValue = 0;
	}
...
FMOD::System * BeatDetector::fmodSetup()
{
	...
	{
		cout << "Error! You are using an old version of FMOD " << version << ". This program requires " << FMOD_VERSION << endl;    return 0;
	}
...
//Sets volume
void BeatDetector::setVolume(float foo)
{
	songChannel1->setVolume(foo);
}

void BeatDetector::setVolumeTwo(float foo)
{
	songChannel2->setVolume(foo);
}

float BeatDetector::getVolume()
{
	float bar;
	songChannel2->getVolume(&bar);
	return bar;
}

//Is supposed to stop the song.
void BeatDetector::unloadSong()
{
	areWePlaying = false;
	//cout << "ARE WE: " << areWePlaying << endl;
	...
	smoothMedian = 0.0f;
	beatThreshold = 0.0f;
	thresholdSmoother = 0.0f;
	dominantValue = 0.0f;
	smootherValues.clear();
	timeToDelay = 0;
	spectrumFluxes.clear();
	dominantValue = 0;
	difference = 0;
	previousFFT = 0;
	completeSong = false;
...
void BeatDetector::pauseSong(bool foo)
{
	songChannel1->setPaused(foo);
	songChannel2->setPaused(foo);
	pausedOrNot = foo;
}

bool BeatDetector::getComplete()
{
	return completeSong;
}
...
//characters
bool BeatDetector::stringValid(const string &str)
{
